...
#####################################
# checking if trend is significant for all years
m1 <- tslm(as.ts(final_data[,"Load"]) ~ trend)
summary(m1) # Trend is significant
#####################################

#####################################
# checking if trend is significant for years 2014-2016
m2 <- tslm(as.ts(training_data[,"Load"]) ~ trend)
summary(m2) # Trend is significant
#####################################
...
# prediction_end_date = end_date + hours(40) 
# 
...
for (i in 1:365){
  ...
  train.ho.a <- ets(ts1, model = "ZZZ")
  #train.ho.m <- holt(train_new, season = "Multiplicative")
  #predictions = predict(train.ho.a,prediction.interval = FALSE)
  #train.ho.a['fitted']
  #autoplot(train.ho.a)
  predictions = predict(train.ho.a,h = 40)
...
#write.csv(predictions_actuals, file = "predictions2018_HoltWinters.csv")
#write.csv(mape_dataset, file = "MAPE2018_HoltWinters.csv")
################################################################
